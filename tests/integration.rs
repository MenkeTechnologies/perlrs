...
//! `ErrorKind` from `eval_err_kind` / `parse_err_kind`. `runtime_extra` adds broad interpreter
//! coverage (assignment forms, builtin return values, aggregates, strings, control flow). Library unit tests cover `parse()`, `run`, lexer (`&`, `&&`/`||`/`+=`, `==`/`!=`,
