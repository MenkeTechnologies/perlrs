...
//! suites (`parse_accepts`, `parse_syntax_errors`, `parse_program_shape`) add explicit `#[test]`
//! cases—no macro batching—for `perlrs::parse` success, syntax errors, and `Program` / `StmtKind`
//! shape. Library unit tests cover `parse()`, `run`, lexer (`&`, `&&`/`||`/`+=`, `==`/`!=`,
