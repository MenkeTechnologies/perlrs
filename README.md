...
**Script files and text reads:** The driver, **`require`**, **`do`**, **`slurp`**, **`par_sed`**, and **`-n` / `-p`** line input decode file and stdin bytes as UTF-8 and replace invalid sequences (U+FFFD) instead of aborting, so scripts or data with occasional non-UTF-8 octets (for example terminal escape bytes embedded in exported env values) still load like they do under stock **`perl`**.
...
- **`par_sed PATTERN, REPLACEMENT, FILES... [, progress => EXPR]`** — read each file as UTF-8 text (invalid UTF-8 is decoded lossily), apply **`PATTERN`** with global substitution semantics (same as **`s///g`**: pass a string pattern or **`qr/.../`** with embedded flags), write back only when the content changes; returns the number of files modified. **`PATTERN`** string uses the same compilation path as regex literals (e.g. **`(?i)`** via **`qr`** for case fold). Optional stderr progress (**`BuiltinId::ParSed`** / **`ParSedProgress`**).
