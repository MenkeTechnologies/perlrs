...
- **JIT** // `src/jit.rs` — Cranelift **method JIT** with two tiers. **Linear JIT**: straight-line (no branches) sequences in one basic block; supports integer and float stack values (int/float promotion matches the VM), returning `i64` or `f64` from native code. **Block JIT**: **integer-only** control flow — loops (`Jump`), conditionals (`JumpIfTrue` / `JumpIfFalse`), short-circuit `&&`/`||` (`JumpIfFalseKeep` / `JumpIfTrueKeep`) — via a CFG with block parameters at merges; if abstract simulation would leave a float on the stack after any data op, the block path falls back to the VM. Unreachable blocks are traps. Shared eligible ops where types allow: `LoadInt`, `LoadFloat`, `LoadConst` (integer or float pool entries), arithmetic, compares, bitwise (integer-only paths), `Div`/`Mod`/`Pow` per validation rules, `Pop`, `Dup`, `Halt`. Slot/plain/arg tables stay `i64` in native code. Not JIT’d: block path with float stack, `LoadUndef`, most non-numeric ops; see `jit.rs` module docs.
